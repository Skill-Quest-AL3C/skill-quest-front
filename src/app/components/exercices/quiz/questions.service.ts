...
export class QuestionsService {
  ...
  public getQuizzes() {
    console.log("coucou2" + this.http.get<any[]>(`./quiz-list.json`))
    return this.http.get<any[]>(`./quiz-list.json`).pipe(
      map((result: any[]) => {
  ...
  public getQuestions(fileName: string) {
    console.log("coucou2" + this.http.get<any[]>(`./${fileName}.json`))
    return this.http.get<any[]>(`./${fileName}.json`).pipe(
      map((result : any[]) => {
        return result.map((r) => new Question(r.question, r.choix));
      })
    );
  }
}
